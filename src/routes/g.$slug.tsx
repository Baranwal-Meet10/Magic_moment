...
/** Confetti particles with dynamic palette, theme emoji support & mobile GPU optimization */
function Confetti({ colors, emojis }: { colors: string[]; emojis?: string[] }) {
  ...
  const pieces = useMemo(() => {
    ...
    return Array.from({ length: count }).map((_, i) => {
      ...
      return {
        emoji: hasEmojis ? emojis[i % emojis.length] : null,
      ...
      {pieces.map((p, i) => (
        <div
          ...
          style={{
            ...
            width: p.emoji ? "auto" : p.width,
            height: p.emoji ? "auto" : p.height,
            fontSize: p.emoji ? "20px" : undefined,
            lineHeight: p.emoji ? 1 : undefined,
            top: -20,
            borderRadius: p.emoji ? undefined : p.radius,
